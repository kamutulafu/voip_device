...
static uvc_fb_t *video_fb_get_cb(void *cb_ctx)
{
    ...
    if (camera_capture_is_requested()) {
        camera_capture_save_frame(uvc->fb.buf, uvc->fb.len);
    }
...
{
    if (!s_uvc_initialized || !s_uvc) {
        return ESP_ERR_INVALID_STATE;
    }
    return video_start_cb(0, CONFIG_UVC_CAM1_FRAMESIZE_WIDTH, CONFIG_UVC_CAM1_FRAMESIZE_HEIGT, CONFIG_UVC_CAM1_FRAMERATE, s_uvc);
}
...
{
    if (s_uvc_initialized && s_uvc && fb) {
        video_fb_return_cb((uvc_fb_t *)fb, s_uvc);
    }
}
