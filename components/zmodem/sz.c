...
void zs_start(char *path)
{
	...
	if (zf == RT_NULL)
	{
	    rt_kprintf("[Zmodem Send] zf: out of memory\r\n");
		return;
	}
	rt_kprintf("[Zmodem Send] Starting send session for file: %s\r\n", path);
    ...
    if (res == RT_EOK)
    {
        rt_kprintf("[Zmodem Send] Transfer completed: file=%s size=%ld bytes\r\n",
		          p,zf->bytes_received);
    }
    else
    {
        rt_kprintf("[Zmodem Send] Transfer failed: file=%s\r\n", p);
    }
	rt_free(zf);
...
static void zsend_init(void)
{
	...
	zinit_parameter();
	rt_kprintf("[Zmodem Send] Initializing parameters, waiting for receiver ZPAD...\r\n");
	for(;;)          /* wait ZPAD */
	{
		res = zread_line(800);
		if (res == ZPAD) break;
	}
	rt_kprintf("[Zmodem Send] Got ZPAD, waiting for ZRINIT header...\r\n");
	for (;;) 
	{
	    res = zget_header(rx_header);
		if (res == ZRINIT) break;
	}
	rt_kprintf("[Zmodem Send] Got ZRINIT header.\r\n");
	...
	rt_kprintf("[Zmodem Send] Handshake complete: Rxflags=0x%02X Txfcs32=%d\r\n", Rxflags, Txfcs32);
    /* send SINIT cmd */
	return;
}
...
static rt_err_t zsend_files(struct zfile *zf)
{
	...
	if (zf->fname == RT_NULL)
	{
	    rt_kprintf("[Zmodem Send] Error: no file to be sent.\r\n");
		return res;
	}
	if ((zf->fd=open(zf->fname, O_RDONLY,0)) <0)
	{
	    rt_kprintf("[Zmodem Send] Can not open file: %s\r\n", zf->fname);
	    return res;
	}
	...
	*q++ = 0;
	...
	rt_sprintf(p, "%lu %lo %o 3 %d %ld", (long)finfo.st_size, (long)finfo.st_mtime,
			  (unsigned int)finfo.st_mode, file_cnt, Left_sizes);
	Left_sizes -= finfo.st_size;
...
loop:
		res = zget_header(rx_header);
		rt_kprintf("[Zmodem Send] zsend_file got header response: %d\r\n", res);
		switch (res) 
		{
		case ZRINIT:
		...
		case ZRPOS:		           	         /* here we want */
		     zget_pos(Rxpos);
			 Txpos = Rxpos;
			 rt_kprintf("[Zmodem Send] Got ZRPOS: receiver requested position %ld. Starting transmission...\r\n", (long)Txpos);
...
start_send:
	rt_kprintf("[Zmodem Send] Sending ZDATA block at position: %ld\r\n", (long)Txpos);
	zput_pos(Txpos);
	...
	do
	{
		...
		rt_kprintf("[Zmodem Send] Sent block size: %d, current Txpos: %ld\r\n", cnt, (long)Txpos);
		if (cmd == ZCRCW)
			goto get_syn1;
	} while (cnt == RX_BUFFER_SIZE);
	for (;;) 	                     /*  get ack and check if send finish */
	{
		rt_kprintf("[Zmodem Send] Sending ZEOF at position: %ld\r\n", (long)Txpos);
		zput_pos(Txpos);
		zsend_bin_header(ZEOF, tx_header);
get_syn1:
        res = zget_sync();
		rt_kprintf("[Zmodem Send] EOF sync response: %d\r\n", res);
		switch (res) 
		{
		case ZACK:
			 goto get_syn1;
		...
		case ZRPOS:				        /* resend here */
		     rt_kprintf("[Zmodem Send] Got ZRPOS: seeking back to position %ld\r\n", (long)Txpos);
		     lseek(zf->fd,Txpos,0);
			 goto start_send;
		case ZRINIT:		           /*  send finish,then begin to send next file */
			 rt_kprintf("[Zmodem Send] File send completed successfully.\r\n");
			 return RT_EOK;
		case ZSKIP:
		case -RT_ERROR:
		     return res;
...
static rt_err_t zget_sync(void)
{
	...
	{
		res = zget_header(rx_header);
		rt_kprintf("[Zmodem Send] zget_sync got header: %d\r\n", res);
		switch (res) 
		{
		case ZCAN:
		case ZABORT:
		...
		case ZRPOS:		             /* get pos, need to resend */
		     zget_pos(Rxpos);
			 Txpos = Rxpos;
			 rt_kprintf("[Zmodem Send] zget_sync reposition to Rxpos: %ld\r\n", (long)Txpos);
			 return res;
		case ZACK:
			 return res;
...
static void zsay_bibi(void)
{
	rt_kprintf("[Zmodem Send] Sending ZFIN (session finish)...\r\n");
	for (;;) 
	{
		...
		{
		case ZFIN:
			rt_kprintf("[Zmodem Send] Got ZFIN ACK. Saying Over and Out (OO).\r\n");
			zsend_line('O'); 
			zsend_line('O'); 
		case ZCAN:
		case TIMEOUT:
